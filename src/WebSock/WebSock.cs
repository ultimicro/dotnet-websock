...
public sealed class WebSock : IAsyncDisposable, IDisposable
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="WebSock"/> class from a completed handshake connection.
    /// </summary>
    /// <param name="connection">
    /// The TCP/IP connection between the endpoint that already completed a WebSocket handshake.
    /// </param>
    /// <exception cref="ArgumentException">
    /// <paramref name="connection"/> nor readable or writable.
    /// </exception>
    /// <remarks>
    /// This class will takeover <paramref name="connection"/>.
    /// </remarks>
    public WebSock(Stream connection)
    {
    ...
    public static async Task<WebSock> ConnectAsync(Uri server, CancellationToken cancellationToken = default)
    {
        using var client = new HttpClient(); // On non-browser the default constructor will always using SocketsHttpHandler.

        return await ConnectAsync(server, client, cancellationToken);
    }

    [UnsupportedOSPlatform("Browser")]
    public static async Task<WebSock> ConnectAsync(Uri server, HttpClient client, CancellationToken cancellationToken = default)
    {
        if (!server.IsAbsoluteUri)
        {
            throw new ArgumentException("The value is not an absolute URL.", nameof(server));
        }

        if (server.Scheme != "ws" && server.Scheme != "wss")
        {
            throw new ArgumentException("The value is not a WebSocket URL.", nameof(server));
        }

        using var handshake = new HttpRequestMessage(HttpMethod.Get, server);

        return await ConnectAsync(handshake, client, cancellationToken);
    }

    [UnsupportedOSPlatform("Browser")]
    public static async Task<WebSock> ConnectAsync(HttpRequestMessage handshake, CancellationToken cancellationToken = default)
    {
        using var client = new HttpClient(); // On non-browser the default constructor will always using SocketsHttpHandler.
    ...
    /// </param>
    /// <returns>
    /// The <see cref="WebSock"/> instance represent the ready to use WebSocket connection.
    /// </returns>
    /// <exception cref="ArgumentException">
    ...
    /// </exception>
    [UnsupportedOSPlatform("Browser")]
    public static async Task<WebSock> ConnectAsync(HttpRequestMessage handshake, HttpClient client, CancellationToken cancellationToken = default)
    {
